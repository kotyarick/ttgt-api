...
pub fn parse() {
	...
	let target_name_selector = Selector::parse(
		r#"font[face="Times New Roman"][size="6"][color="\#ff00ff"]"#,
	)
	.unwrap();
		...
		for table in html.select(&table) {
			...
			for row in table.select(&row).skip(2) {
				...
				for cell in cells.skip(1) {
					...
					if peek.is_none_or(|text| {
						matches!(text.as_str(), "" | "-" | "_" | " ")
					}) {
					...
					match target_type {
						...
						ItemType::Group => {
							...
							let second_line = text
								...
								.split_whitespace()
								.collect::<Vec<&str>>();
							let lesson = match text.next().map(|text| {
